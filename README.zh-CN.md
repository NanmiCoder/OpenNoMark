...
OpenNoMark 已按最新 [`skills`](https://github.com/vercel-labs/skills) 标准提供跨 Agent
Skill。可以通过 NPM 安装到 Codex、Claude Code、Cursor、OpenCode 等 Agent：
...
Skill 保持为轻量、可移植的指令层，通过已安装的 Python CLI 或 `uvx` 调用同一套
图像处理实现，避免不同 Agent 各自维护一份去水印逻辑。
...
├── skills/                  # 跨 Agent Skills（vercel-labs/skills 布局）
│   └── opennomark/
│       ├── SKILL.md
│       └── agents/openai.yaml
├── examples/                # 示例图片（含水印原图）
