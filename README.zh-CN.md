...
OpenNoMark 将水印定位与 LaMa 内容感知修复结合起来，面向 Gemini、豆包、千问、即梦、可灵、腾讯元宝、百度等生成图片中常见的可见角落水印，完整处理流程都运行在你自己的电脑上。
...
- **数据校准定位：** Gemini 使用真实正样本与困难负样本校准轻量空间检测器，并没有微调基础模型；开放词汇专家组合 `watermark` 与 `brand watermark` 候选，再用贴边距离、文字形态、置信度排序与覆盖式去重筛选。当两个专家同时命中时，系统会综合同角位置、重叠度、置信度与贴边距离仲裁，避免类似星芒的背景纹理盖过真正的文字签名。
